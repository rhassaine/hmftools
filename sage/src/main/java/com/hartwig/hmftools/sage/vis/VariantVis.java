...
import com.hartwig.hmftools.sage.vis.SvgRender.RenderedGeneData;
...
public class VariantVis
{
            ...
            final VisConfig config, @Nullable final ReferenceData refData)
    {
        ...
        Stream<DomContent> tumorReadTableRows = tumorVis.stream().map(x -> x.renderReads(true, aaElements)).flatMap(x -> x.stream());
        Stream<DomContent> refReadTableRows = refVis.stream().map(x -> x.renderReads(false, aaElements)).flatMap(x -> x.stream());
        ...
        String htmlStr = html(
                ...
                body(
                        firstVis.renderVariantInfo(
                                ...
                                referenceIds),
                        verticalSpacer,
                        styledTable(List.of(tr(
                                td(renderSampleSummaryAndCountsTable(tumorReadCounters, refReadCounters, tumorIds, referenceIds)).withStyle(CssBuilder.EMPTY.verticalAlign("top").toString()),
                                td(renderSampleQualsAndSiteInfo(tumorReadCounters, refReadCounters, tumorIds, referenceIds, firstCounter)).withStyle(CssBuilder.EMPTY.verticalAlign("top").toString()),
    ...
    // TODO: review
    public static DomContent renderSampleSummaryAndCountsTable(final List<ReadContextCounter> tumorReadCounters,
            final List<ReadContextCounter> refReadCounters, final List<String> tumorIds, final List<String> refIds)
    {
        ...
        for(int i = 0; i < contentRowsElems.size(); i++)
        {
            ...
            if(i == 3)
            {
                rows.add(tr(td("Support By Type").withStyle(cellStyle.noBorder().toString()).attr("colspan", colCount)).withStyle(headerStyle.toString()));
            }
            ...
            {
                contentRowElems.set(j, contentRowElems.get(j).withStyle(CssBuilder.EMPTY.textAlign("right").merge(cellStyle.merge(borderStyle)).toString()));
            }
    ...
    // TODO: review
    public static DomContent renderSampleQualsAndSiteInfo(final List<ReadContextCounter> tumorReadCounters,
            final List<ReadContextCounter> refReadCounters, final List<String> tumorIds, final List<String> refIds,
            final ReadContextCounter firstCounter)
    {
        ...
        tumorValue = tumorCounter == null ? null : String.valueOf(tumorCounter.altSupport() > 0 ? (int) Math.round(tumorCounter.altMapQualityTotal() / (double) tumorCounter.altSupport()) : 0);
        refValue = refCounter == null ? null : String.valueOf(refCounter.altSupport() > 0 ? (int) Math.round(refCounter.altMapQualityTotal() / (double) refCounter.altSupport()) : 0);
        ...
        tumorValue = tumorCounter == null ? null : format("%d", tumorCounter.fragmentCoords().minCount());
        refValue = refCounter == null ? null : format("%d", refCounter.fragmentCoords().minCount());
        ...
        for(int i = 0; i < contentRowsElems.size(); i++)
        {
            ...
            for(int j = 1; j < contentRowElems.size(); j++)
            {
                contentRowElems.set(j, contentRowElems.get(j).withStyle(CssBuilder.EMPTY.textAlign("right").merge(cellStyle).toString()));
            }
    ...
    // TODO: review
    private DomContent renderVariantInfoTable(int totalTumorQuality, double mapQualFactor, boolean nearbyIndel, double maxDistanceFromEdge,
            double nonAvgEdgeDist, double altAvgEdgeDist, final Set<String> filters, @Nullable final AminoAcidElements aaElements)
    {
        CssBuilder borderStyle = CssBuilder.EMPTY.border(CssSize.px(1.0), "solid", Color.BLACK).borderCollapse("collapse");
        ...
        if(!filters.isEmpty())
            filterStr = filters.stream().collect(Collectors.joining(","));
        ...
        List<DomContent> rows = Lists.newArrayList();
        for(int i = 0; i < rowsElems.size(); i++)
        {
            List<TdTag> rowElems = rowsElems.get(i);
            ...
            final BaseRegion viewRegion, final SageVariant sageVariant, final RefGenomeSource refGenome)
    {
        ...
        RenderedGeneData renderedGeneData = renderAminoAcids(viewRegion, transcriptExons, transcriptAminoAcids, aaEvents, refGenome, sageVariant);
	...
	// TODO: review
        ...
        if(aaElements != null)
        {
            ...
            DomContent aaPredictedRow = tr(td("predicted").attr("colspan", columns.size() + 1).withStyle(headerStyle.toString()), td(aaElements.alt));
            tableRows.add(aaPredictedRow);

            // gene name row
            CssBuilder geneNameStyle = CssBuilder.EMPTY.fontSizePt((int) Math.round(2.0 / 3.0 * READ_HEIGHT_PX)).fontWeight("bold").textAlign("center");
    ...
    // TODO: review
    private static RenderedGeneData renderAminoAcids(final BaseRegion viewRegion, final TranscriptData transcriptExons,
            ...
            final SageVariant variant)
    {
        ...
        List<GeneRegionViewModel> altViewModels = getAltGeneRegionViewModels(transcriptExons, transcriptAminoAcids, geneRegions, events, viewRegion, refGenome, variant);
        return renderGeneData(READ_HEIGHT_PX, viewRegion, transcriptAminoAcids.GeneName, posStrand, refViewModels, altViewModels);
    }
    ...
    private void downsampleReadEvidenceRecords()
    {
        ...
        for(Map.Entry<ReadContextMatch, List<ReadEvidenceRecord>> entry : mReadEvidenceRecordsByType.entrySet())
        {
            ...
            while(!records.isEmpty() && newRecords.size() < maxReads)
            {
                ...
                // TODO: use ThreadLocalRandom.current()
